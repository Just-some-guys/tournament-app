...
namespace TournamentApp.Application.Services.Brackets
{
    public class BracketService : IBracketService
    {
        ...
        {
            Tournament tournament = new Tournament(); // Должен передаваться как параметр
            tournament.BracketType = BracketType.DoubleElimination;  // Должен передаваться как параметр
            Bracket result = new Bracket();    
        
            if (tournament.BracketType == BracketType.DoubleElimination)
            {
                DoubleEliminationModel branches = GetDEModelAutoNew();

                result.UpperBranch.Matches = branches.UpperBranch;
                result.LowerBranch.Matches = branches.LowerBranch;
            ...
            if (tournament.BracketType == BracketType.SingleElimination)
            {
                ...
                result.UpperBranch.Matches = branches.UpperBranch;
                result.LowerBranch.Matches = null;
                result.Tournament = tournament;
            }
